...
const getKledingstukById = async (id, auth0) => {
    ...
    if (!kledingstukById) {
        throw ServiceError.notFound(`kledingstuk met id ${id} bestaat niet`, {
            id
        });
    }
    ...
    if (eigenaar.auth0id !== auth0) {
        throw ServiceError.notFound(`Je hebt geen toegang tot dit kledingstuk`, {
            id
        });
    }
...
const create = async ({
    ...
    let newKledingstuk = await kledingstuk.create({
        brand,
        color,
        type,
        size,
...
const updateKledingStukById = async (id, {
    brand,
    color,
    type,
    size,
