 ...
 const getAll = async () => {
     return await kleerkast.findAll({
         include: ['kledingstukken']
     }).then((kleerkasten) => {

         return {
             kleerkasten: kleerkasten,
             lengte: kleerkasten.length
         };
     })
 };
 ...
 const getKleerkastById = async (id, auth0) => {
     ...
     if (!kleerkastById) {
         throw ServiceError.notFound(`kleerkast met id ${id} bestaat niet`, {
             id
         });
     }
     let eigenaar = await kleerkastById.getUser();
     if (eigenaar.auth0id !== auth0) {
         throw ServiceError.notFound(`Je hebt geen toegang tot deze kleerkast`, {
             id
         });
     }
 ...
 const updateKleerkastById = async (id, {
     name,
     location,
     userId
 }, auth0) => {
     ...
     const existingKleerkast = await kleerkast.findOne({
         where: {
             name,
             location: location,
             userId: userId
     ...
     if (existingKleerkast) {
         throw ServiceError.validationFailed(`kleerkast met naam ${name} en locatie ${location} bestaat al`, {
             name,
             location
         });
     }
     ...
     if (!existingUser) {
         throw ServiceError.notFound(`user met id ${userId} bestaat niet`, {
             userId
         });
     }
 ...
 const getKledingstukkenByKleerkastId = async (kleerkastId, auth0) => {
     ...
     if (!kleerkastById) {
         throw ServiceError.notFound(`kleerkast met id ${kleerkastId} bestaat niet`, {
             kleerkastId
         });
     }
     let eigenaar = await kleerkastById.getUser();
 ...
 const create = async ({
     ...
     const newKleerkast = await kleerkast.create({
         name,
         location,
         userId
     });
 ...
 module.exports = {
     getAll,
     getKleerkastById,
     getKledingstukkenByKleerkastId,
     updateKleerkastById,
     deleteById,
     create,
     belongsToUser
